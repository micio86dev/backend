...
return [
     ...
     */
    // Array form so `api/test-isolation/*` — loaded ONLY when APP_ENV=testing
    // (AppServiceProvider::boot(), routes/api-test-isolation.php) — never
    // reaches the exported document. CI's job-level `APP_ENV: testing` env
    // var applies to every step including `scramble:export`, so without this
    // exclusion a testing-env export would always include these test-only
    // routes while the committed snapshot (generated by a developer outside
    // APP_ENV=testing) never does — making the freshness gate permanently
    // red regardless of resource correctness. Discovered during the D2
    // determinism probe (Phase 1, generated-client-truth-and-session-safety).
    'api_path' => [
        'include' => 'api',
        'exclude' => ['api/test-isolation'],
    ],
