...
/**
 * Thrown when an indicator score is outside the legal domain {1,3,5,-1}.
 *
 * Values 2, 4, decimals, or any other out-of-set value trigger this.
 * The caller marks the competency as llm_parse_error.
 * MUST NOT coerce the score; MUST NOT trigger a queue retry.
 *
 * REQ: IndicatorValidator — score ∈ {1,3,5,-1} (C9 D4)
 */
final class InvalidIndicatorScoreException extends \RuntimeException
{
    public function __construct(int $score, int $position = -1)
    {
        ...
        parent::__construct(
            "Invalid indicator score [{$score}]{$ctx}: must be one of {1, 3, 5, -1}."
        );
    }
}
